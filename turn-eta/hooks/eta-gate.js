...
const MS = 60000
const STALE_MS = 4 * MS // a turn older than this was abandoned. Same window eta.js uses.
  ...
  for (const provider of subdirs(base)) {
    for (const model of subdirs(path.join(base, provider))) {
      ...
      for (const line of text.split('\n')) {
        ...
        const f = line.split('\t')
        if (f[6] === 'open' && now - Date.parse(f[1]) < STALE_MS) return true
      }
    }
  }
  return false
...
function selftest() {
  ...
  fs.writeFileSync(log, '# header\n' + row('aaa', new Date(t0 - 9 * MS).toISOString(), 'open') + '\n')
  prompt()
  assert.ok(edit(), 'a stale open turn is not a running plan')
