...
어느 에이전트든, 어느 프로바이더든, 어느 모델이든 씁니다.
...
마지막 줄이 이 스킬의 전부입니다. 첫 예상은 남이 정해 준 기본값이고 열 번째 예상은 내 컴퓨터, 내 코드,
내가 실제로 쓰는 모델에서 잰 값입니다.
...
20턴까지 봅니다. 어느 단을 썼는지 늘 같이 찍혀서 숫자가 어디서 나왔는지 보입니다.
