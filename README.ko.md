...
<p align="center">
  <img src="assets/demo-turn.gif" alt="A turn opening with a numbered plan and a finish time, the ETA moving as each step lands" width="880">
</p>
...
그 스텝이 오래 걸려서 끝날 시각이 같이 밀렸습니다. 이게 도는 동안 보이는지 끝나고 한꺼번에 보이는지는
...
| **정직한 마무리** | 답변을 끝난 시각으로 닫습니다. 시계가 이미 틀렸다고 판정한 예상을 다시 적지 않습니다. |
...
턴이 도는 동안에는 이 기록과 이번 턴의 실제 속도를 섞어서 냅니다. 섞는 비율은 얼마나 끝냈느냐입니다.
4스텝 중 1스텝을 끝냈을 땐 기록이 4분의 3을 먹고, 3스텝을 끝냈을 땐 이번 턴이 4분의 3을 먹습니다. 그래서
첫 스텝이 느려도 끝날 시각이 움직이기만 하지 통째로 끌려가지 않습니다.
...
계획 없는 입력이 잔손질 크기를 넘어선 뒤에야 `Edit`·`Write`를 한 번 막습니다 — 프롬프트가 온 지
6분이 지났는데 아직 고치고 있거나, 파일을 여섯 번째 고칠 때. 한 번뿐이고 재시도는 어느 쪽이든
통과합니다. 두 기준 아래면 아무 말도 안 하니, 한 줄 고치는 데 알림값을 치르지 않습니다. 리팩터링과
오타 수정은 둘 중 하나가 벌어지기 전에는 구별할 수 없어서, 턴이 어느 쪽인지 스스로 드러낼 때까지
기다립니다. 턴이 끝날 때는 빠뜨린 `done`을 물어봅니다. `plan`은 돌았는데 그 내용이 답변에 안 들어갔으면 답변을
